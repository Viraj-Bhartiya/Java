...
void randomize () {
  a=1f;
  b=a;
  m=(int)random (1, 50); //has to be an integer, otherwise the shape will be open 
  n1=random (0.1f, 30f);
  n2=random (0.1f, 30f);
  n3=random (0.1f, 30f);
}
...
void drawShapePointsInt(int radialSteps, int internalSteps, int pointColor, float outerWeight, float innerWeight) {
  ...
  for (int p = 0; p < radialSteps; p++) {
    ...
    for (int i = 0; i<internalSteps; i++) {
      ...
      float weight = map (i, 0, internalSteps-1, outerWeight, innerWeight)*1.3;
      float alpha = map (i, 0, internalSteps-1, 255, 1);
      stroke (pointColor,i, alpha);
      strokeWeight (weight);
